...
Timestamp EPollPoller::poll(int timeoutMs,ChannelList *activeChannels)
{
    ...
    int numEvents = ::epoll_wait(this->epollfd_,&*this->events_.begin(),static_cast<int>(this->events_.size()),timeoutMs);
    int saveErrno = errno;

    Timestamp now(Timestamp::now());
    if(numEvents > 0){
        LOG_DEBUG("events happend:%d",numEvents);
        fillActiveChannels(numEvents,activeChannels);
        if(numEvents == this->events_.size()){
...
void EPollPoller::fillActiveChannels(int numEvents,ChannelList *activeChannels) const
{
    for(int i = 0;i < numEvents;++i){
        ...
        channel->set_revents(this->events_[i].events);
